...
from data.renderloader import create_loader_render
...
#from MVFractalDB import MVFractalDB, worker_init_mvfdb_fn
...
parser.add_argument('--entity-name', default='daweek', type=str,
...
def main():
    ...
    # args.distributed = int(os.getenv('MPI_COMM_WORLD_SIZE', '0')) > 1
    args.distributed = True
    args.local_rank = 0
    args.world_size = 1
    args.rank = 0  # global ranks
    if args.distributed:
        ...
        ip = sp.getoutput('/usr/sbin/ip a show dev tofu0 | grep -w inet | cut -d " " -f 6 | cut -d "/" -f 1')
         # Initialize torch distrobuted method
        dist.init_process_group("mpi", init_method="env://")
        print0("Benchmark on Fugaku - Only Train on ImageNet using .tar files")
        
        # Get info about distributed job from enviroment
        rank        = dist.get_rank()
        world_size  = dist.get_world_size()
        print("Hello from dist_rank:{}, dist_world: {} \n".format(rank,world_size))
    ...
    if args.distributed:
        if has_apex and use_amp != 'native':
            ...
            model = NativeDDP(model)  # can use device str in Torch >= 1.1
    ...
    if args.webdataset:
    ...
    elif args.render_fdb or args.render_mvfdb:
        print0("\n\n===< CPU_RENDER Loading DataSet with cpu render....")
        # create datasets with CPU render dataloader

        if args.render_fdb:
            print0("===>   FDB_render  ....")
            dataset_train = Fractal2D_cpu(root=args.csv, width = 256, height = 256, npts = 100000, patch_mode = 0, patch_num = 10, patchgen_seed = 100, pointgen_seed = 100 
            # # transform=transform   -> We comment out since the transformas are hapening on the loader.
            )
            w_i_fn = worker_init_fdb_fn

        elif args.render_mvfdb:
            print0("===>   MV FDB_render  ....")
            dataset_train = MVFractalDB(root=args.csv,
                                    # transform=transform, -> We comment out since the transformas are hapening on the loader.
                                    # params_file = args.data_path
                                    npts = 100000
                                    ,paramtrans_grid = 3
        ...
        if args.no_aug or not train_interpolation:
            train_interpolation = data_config['interpolation']
        loader_train = create_loader_render(
            dataset_train,
            input_size=data_config['input_size'],
            batch_size=args.batch_size,
            is_training=True,
            ...
            pin_memory=args.pin_mem,
            repeated_aug=args.repeated_aug,
            worker_init_fn=w_i_fn  #-> Worker init function for render
        )
