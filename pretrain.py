...
parser.add_argument('--project-name', default='fugaku', type=str,
...
def main():
    ...
    if args.distributed:
        ...
        _logger.info('Training in distributed mode with 1 processes, 1 CPU, Process %d:%d, total %d.'
                     % (args.local_rank, node, args.world_size))
    else:
        _logger.info('Training with a single process on 1 CPUs.')
    assert args.rank >= 0
    ...
    if args.webdataset:
        ...
        loader_train = wds.WebLoader(train_dataset, batch_size=None, shuffle=False)
        ...
        print0("dataset_size:{}, batch_size:{}, world_size(Total Devices):{}".format(dataset_size, args.batch_size,
                                                                                        args.world_size))
        print0("----> Number of batches to be processed per CPU = {}".format(number_of_batches))
